...
function createBoxes(amount) {
  ...
  if (boxes.children.length !== 0) {
    if (
      confirm(`Колекція містить створених ${boxes.children.length} блоків. Для продовження необхідно очистити колекцію.
        Очистити колекцію?`)
    )
      destroyBoxes();
    else return;
  ...
  for (let i = 1; i <= amount; i++) {
    let elmMarkup = "";
    elmMarkup = `<div style="position: absolute; z-index: ${
      amount - i
    }; background-color: ${getRandomColor()}; height: ${getBoxSize(
      i
    )}px; width: ${getBoxSize(i)}px"></div>`;
